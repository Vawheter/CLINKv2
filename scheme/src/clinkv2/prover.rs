...
use math::{
    AffineCurve, Group, One, PairingEngine, PrimeField, Field, ProjectiveCurve, UniformRand, Zero,
};
...
impl<E: PairingEngine> ProvingAssignment<E> {
    ...
    -> Result<Proof<E>, SynthesisError>
    {
        ...
        for j in 0..m_mid {
            ...
            let (rj_comm, rj_rand) = KZG10::<E>::commit(&kzg10_ck, &rj_poly, hiding_bound, Some(&mut rng))?;
        ...
        let opening_challenge = E::Fr::rand(&mut rng);
        let r_mid_q_proof = KZG10::<E>::batch_open(&kzg10_ck, &r_q_polys[m_io..], zeta, opening_challenge, &r_mid_q_rands)?;
    ...
    ) -> Result<bool, SynthesisError>
    {
        ...
        transcript.append_message(b"witness polynomial commitments", as_bytes(&proof.r_mid_comms));
        let mut c = [0u8; 31];
        transcript.challenge_bytes(b"batching challenge", &mut c);
        let eta = E::Fr::from_random_bytes(&c).unwrap();
        ...
        assert!(KZG10::<E>::batch_check(&kzg10_vk, &r_mid_q_comms, zeta, &proof.r_mid_q_values, &proof.r_mid_q_proof, proof.opening_challenge)?);
