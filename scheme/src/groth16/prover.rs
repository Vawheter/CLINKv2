    ...
    C: ConstraintSynthesizer<E::Fr>,
{
    ...
    let mut h_ifft_time = Duration::new(0, 0);
    ...
    // Compute A
    let mut A_time = Duration::new(0, 0);
    let start = Instant::now();
    ...
    println!("A_time: {:?}", A_time);


    // Compute B in G1 if needed
    let mut B_G1_time = Duration::new(0, 0);
    let start = Instant::now();
    ...
    B_G1_time += start.elapsed();
    println!("B_G1_time: {:?}", B_G1_time);

    // Compute B in G2
    let mut B_G2_time = Duration::new(0, 0);
    let start = Instant::now();
    ...
    B_G2_time += start.elapsed();
    println!("B_G2_time: {:?}", B_G2_time);


    let mut C_time = Duration::new(0, 0);
    let start = Instant::now();
    ...
    let a = g_a.into_affine();
    let b = g2_b.into_affine();
    let c = g_c.into_affine();

    let proof_size = mem::size_of_val(&a)
                    + mem::size_of_val(&b)
                    + mem::size_of_val(&c);
    println!("{:?}", proof_size);
